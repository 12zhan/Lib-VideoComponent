...
// Shared body for both Video overloads. The optional playing state selects
...
View VideoImpl(const VideoSource& source, const State<bool>* playing, VideoOptions options) {
  ...
  // Allocated unconditionally so the state slot identity stays stable when
  // the caller switches between controlled and uncontrolled use.
